...
DEV=(function(){
	...
	function initIndex(){
		...
		function chapter(list, lID, sID){
			...
			UTIL.forIn(list,function(b1,b2){
				...
				if(b1.icon != ''){
					...
					_li.append(_title);
					_title.attr('data-chapter',cID);
					_title.addClass('title');
					
					_title.innerHTML='<span data-icon="'+b1.icon+'">'+b1.icon+'</span><span class="title">'+b1.title+'</span>';/*<span class="page">'+b1.page+'쪽</span>*/
					_title.chapterObj=b1;
					_title.bookPage=b1.page;
					
					/*이벤트 주기*/
					_title.off().on('click',click);
					
					if(b1.list&&b1.list.length>0){
						_li.removeClass('blank');
						/*선택 표시*/
						if(sID==cID) _li.addClass('on');
						else _li.removeClass('on');
						/*모든 코너 등록*/
						corner(_li, b1.list, cID);
					}else{
						_li.addClass('blank');
						_li.removeClass('on');
					};
		...
		function corner(_chaper, list, cID){
			if(!list) return;
			/*챕터 컨테이너*/
			...
			UTIL.forIn(list,function(b1,b2){
				...
				_li.innerHTML='<span data-icon="'+b1.icon+'">'+b1.icon+'</span>'
								+'<span class="title">'+b1.title+'</span>'
	...
	function initMenu(){
		...
		/*
		document.querySelector('#quick-book .currect>input').on('keyup',function(event){
			...
			var p=this.value;
			this.value=p.replace(/[^0-9]/g, "");
		});
		
		document.querySelector('#quick-book button[data-type="goBook"]').on('click', function(){
			gotoPage();
		});
		*/
		function gotoPage(){
			var cnt=parseInt(document.querySelector('#quick-book .currect>input').value);
			var max=parseInt(document.querySelector('#quick-book .total').text());
